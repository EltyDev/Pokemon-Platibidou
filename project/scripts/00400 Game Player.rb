class Game_Player < Game_Character
    ...
    def update
        unless @freeze # If the player is not frozen
            Online.update_position() # Update the position of the player on the server
            return send(@update_callback) if @update_callback
            last_moving = moving?
            if moving? || $game_system.map_interpreter.running? ||
                @move_route_forcing || $game_temp.message_window_showing || @sliding
